...
class MemoryTransformerEncoderLayer_MEMABSPOS_SINGLESHOT_AdaLN(nn.Module):
        ...
        if self.norm_first:
            if info is not None:
                with info(name_scope = f"self_attn"):
                    seq_out, mem_out = self._sa_block(self.norm1(seq, depth),self.norm1(mem, depth), src_mask, src_key_padding_mask, info = info)
            else:
                seq_out, mem_out = self._sa_block(self.norm1(seq, depth),self.norm1(mem, depth), src_mask, src_key_padding_mask)
            seq, mem = seq + seq_out, mem + mem_out # (B, L, N), (B, M, N)
            
            seq_mem = torch.cat((seq, mem), dim = 1) # B, L+M, N
            seq_mem = self._ff_block(self.norm2(seq_mem, depth)) # B, L+M, N
            ...
            seq, mem = self.norm1(seq + seq_out, depth), self.norm1(mem + mem_out, depth) # (B, L, N), (B, M, N)
            
            seq_mem = torch.cat((seq, mem), dim = 1) # B, L+M, N
            seq_mem = self._ff_block(seq_mem)  # B, L+M, N
            seq, mem = self.norm2(seq + seq_mem[:, :seq.shape[1], :], depth), self.norm2(mem + seq_mem[:, -mem.shape[1]:, :], depth) # (B, L, N), (B, M, N)
