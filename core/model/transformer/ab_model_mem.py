...
class Model(nn.Module):
    def __init__(self, encoder_layer, num_layers:int, hidden_size:int, threshold:float = 0.9, max_seq_len:int=10000, norm = None):
        ...
        self.enc = encoder_layer.to("cuda:0")
        self.enc2 = copy.deepcopy(self.enc).to("cuda:1")
        self.norm = norm
    ...
    def forward(self, state: Tensor, mem: Tensor, mask=None, src_key_padding_mask=None,  info = None):
        ...
        while(condition and step < self.max_hop):
            ...
            if step > (self.max_hop // 2):
                state = state.to("cuda:1")
                timing_signal = timing_signal.to("cuda:1")
                mem = mem.to("cuda:1")
                encoder = self.enc2
                previous_state = previous_state.to("cuda:1")
            else:
                timing_signal = timing_signal.to("cuda:0")
                previous_state = previous_state.to("cuda:0")
                encoder = self.enc 
                
            state = state + timing_signal
            mem = mem + timing_signal
            
            p = self.p(state.to("cuda:0")).squeeze(-1)
            ...
            if step > (self.max_hop // 2):
                update_weights = update_weights.to("cuda:1")
            
            condition = torch.any(torch.lt(halting_probability,self.threshold))
            
            state, mem = encoder(state, mem, src_mask=mask, src_key_padding_mask=src_key_padding_mask)
