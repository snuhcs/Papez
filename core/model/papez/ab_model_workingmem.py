...
class Dual_Path_Model(nn.Module):
        ...
        **kwargs,
        ...
        self.dislodge = nn.Sequential(
                            ...
                            nn.Conv1d(
                              ...
                              out_channels = (out_channels-1)* self.num_spks,
                              kernel_size = 1,
                            ),
                            nn.Tanh()
                        ).to("cuda:1")
    ...
    def forward(self, x, mem, info = None):
        ...
        B, _, L = x.shape
        return torch.reshape(x, (B, self.num_spks, self.out_channels-1, L)) # B, spks, N, L
...
class Model(nn.Module):
        ...
        **kwargs,
        ...
        self.encoder = nn.Sequential(
                          ...
                          nn.Conv1d(
                            ...
                            out_channels = encoder_out_nchannels-1,
                            kernel_size = 1,
                          ),
                        ).to("cuda:0")
        ...
        self.decoder = nn.Sequential(
                            nn.Conv1d(
                              in_channels = encoder_out_nchannels-1, 
                              out_channels = encoder_out_nchannels,
                              kernel_size = 1,
                            ),
                            nn.InstanceNorm1d(encoder_out_nchannels, affine=True),
        ...
        self.register_buffer('memory_slots',self._memory_slots(encoder_out_nchannels, num_memory_slots)) # 1, N, M 
    ...
    def forward(self, mix, info = None):
        ...
        mix_w = self.encoder(mix) # B, N-1, L'
        mix_w_tag = F.pad(mix_w, (0,0,0,1), "constant", 0) # Sequence Tag  (B, N, L')
        memory_slots = self.memory_slots.repeat(B,1,1).to("cuda:0")# B, N, M
        ...
        sep_h = mix_w.to("cuda:1") * est_mask  # B, n_spks, N-1, L'
