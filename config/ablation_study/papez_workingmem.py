...
from core.model import AB_Papez_WorkingMem, AB_Trans_Mem, AB_MemoryTransformerEncoderLayer
...
config.update(Munch(
    ...
    model = ModuleDict(
        module = AB_Papez_WorkingMem,
        submodules = Munch(
            intra_model = ModuleDict(
                module = AB_Trans_Mem,
                submodules = Munch(
                    encoder_layer = ModuleDict(
                        module = AB_MemoryTransformerEncoderLayer,
                        d_model=token_dim, 
                        nhead=8, 
